...
**v0.7.0-draft** — 2026-07-30 — Sourced from the Collabrium brand deck
...
**Live gallery — `preview.html`, Components tab (v0.7.0).** Every
...
**Scope note:** 17 components are specced and built now — the original
7 basics (Button, Input field, Card, Badge & Tag, Table row, Modal /
dialog, Empty state) plus 10 more transcribed directly from the
teammate's real component source in v0.7.0 (SidebarNav, Tabs, Select,
Checkbox, Radio, Switch, Toast, Tooltip, DataTable, ElementBadge) — see
each section below for source notes. **Still not specced or built:**
stat/KPI cards, filters, pagination, and date pickers (dashboard
components with no source in either the original brand deck or the
teammate's build — these need an actual design pass, not a
transcription) and chart color mapping (needs a real chart library
decision first). Don't skip straight to markup for a new component —
write the spec here first (variants, sizes, states, Do/Don't), the same
process every component above went through.
...
these read as deliberate at small sizes.
