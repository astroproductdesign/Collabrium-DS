...
service for one. Chat window is literally the expanded form of Prompt
input bar — clicking the bar's Floating variant morphs it into the
window via a shared FLIP animation (see Chat window's own Behavior
section, below). Loading State and Thinking are the two pre-result
wait states either surface shows while that AI interaction is running,
before anything has streamed back — which one appears is decided by
Thinking's own Rule 2 gate (below), never by preference. Streaming
Text is what either of those hands off to at first token: the model's
own prose actually arriving, not a wait state standing in for it.
...
- **Rule 6 — It vanishes completely at first token.** Not at completion — at the first token. Pre-first-token is this component; streaming is [Streaming Text](#ai-native). It leaves nothing behind: no record, no past-tense label, no residue. A loader still animating beside arriving text is the commonest defect in this pattern.
...
**Permission Prompt.** ⚠️ **Designed from scratch** — no source in
either the original brand deck or the teammate's build.

An AI Native human-in-the-loop card the AI surfaces inline — appended
directly into its own turn in the thread — to ask authorization for a
specific, consequential action before it proceeds: reading or writing
a file, running a command, sending a message on the person's behalf,
calling an external API. Unlike [Tool approval](#ai-native), which is
a separate floating card pinned above the composer, Permission Prompt
is inline: a genuine part of the AI's own message, never an overlay
and never something that blocks the composer or the rest of the
thread. It reuses Card, Badge's status-color variants, Button, and
Thinking's own collapsible-detail technique throughout rather than
inventing new controls. Class prefix: `.c-permission-prompt*`.

**Permission Prompt — Anatomy (head).**

| Part | Spec |
|---|---|
| Root (`.c-permission-prompt`) | Card shell reused verbatim (`.c-card`), `max-width: 360px` standalone default — a host inside Chat window overrides this (Rule 9) |
| Head (`.c-permission-prompt-head`) | Flex row, `align-items: center` — icon, label, and risk badge share one vertical center |
| Icon (`.c-permission-prompt-icon`) | Represents the action's own type (a file, a terminal command, an email, an API call), Neutral-7, `icon-md` |
| Label (`.c-permission-prompt-label`) | Primary font, heading-5 size/line-height/weight, Neutral-9 — the question itself, e.g. "Send email on your behalf?" |
| Risk badge | Real Badge status variants reused verbatim — Low/Medium/High risk maps to success/warning/error (green/amber/red), no new colors invented. Pinned to the same top-right corner in both the pending and resolved states (Rule 5) |

**Permission Prompt — Anatomy (detail, scope, actions).**

| Part | Spec |
|---|---|
| Toggle (`.c-permission-prompt-toggle`) | "What this will do" while pending, "View details" once resolved — expands/collapses the detail block; its chevron rotates 180° when expanded |
| Detail (`.c-permission-prompt-detail`) | Thinking's own `0fr → 1fr` grid-rows + opacity expand/collapse, verbatim. Structured key:value rows only — never raw JSON (Rule 4). Values never truncate or ellipsis; a long one wraps instead |
| Scope note (`.c-permission-prompt-scope`) | One caption line distinguishing what "Allow once" covers from what "Allow for session" covers |
| Actions (`.c-permission-prompt-actions`) | Three real Button instances — Deny (Ghost + a scoped red color override), Allow once (Secondary), Allow for session (Primary) — right-aligned, natural width, one row on desktop (Rule 6) |
| Resolved state (`.c-permission-prompt-resolved`) | Label (body2) + a status badge (Allowed/Denied/Expired) in the same corner the risk badge occupied. The card stays in the transcript — it never disappears (Rule 7) |

**Permission Prompt — Rules.** Eleven rules, not stylistic preference —
each one closes off a specific way a human-in-the-loop authorization
card has been seen to fail.

- **Rule 1 — Scope: authorization for a specific action, not a general confirmation.** Appears only when the AI is about to do something with a real consequence outside the conversation itself — read or write a file, run a command, call an external API or service, send something on the person's behalf. Never used to confirm something that only affects the conversation itself.
- **Rule 2 — Inline, not a modal.** Appended directly into the AI's own turn in the thread — a genuine part of that message, never an overlay, scrim, or anything that blocks the composer or the rest of the thread.
- **Rule 3 — The detail defaults open while pending, and stays reachable once resolved.** The person sees exactly what's being authorized before deciding, without an extra click; after a decision, the same toggle still opens it, just collapsed by default now that the decision itself is the headline.
- **Rule 4 — Every request states exactly what it authorizes — structured, never raw.** The detail block is always key:value rows, never a JSON dump; values are shown in full, never truncated, since the entire point is showing exactly what's being authorized.
- **Rule 5 — Risk is always visible, pending or resolved, in the same corner.** The badge lives in the same top-right position of the head row in both states, so the eye never has to relearn where to look.
- **Rule 6 — Three actions, escalating commitment, deny-first.** Left to right on desktop: Deny (Ghost), Allow once (Secondary), Allow for session (Primary) — transparent to outlined to solid, the same visual-weight-as-commitment convention this system already uses, and the same deny-first ordering Claude Code's own prompts use.
- **Rule 7 — A resolution is permanent, and it stays visible.** Once Denied or Allowed, the card collapses to a compact record but never disappears — it's the audit trail for what was actually authorized. This is a deliberate contrast with [Tool approval](#ai-native)'s own Rule 13: that flow closes outright once resolved, because it's answering questions, not authorizing consequential actions. Reopening or undoing a resolved decision isn't offered; a new request is a new card.
- **Rule 8 — Expired is a session boundary, not a decision.** A request that goes unanswered before the conversation moves on shows Expired (a neutral badge) — nothing runs, and no control reaches this state directly; only the conversation's own progression does.
- **Rule 9 — Width matches the host.** `360px` standalone; inside Chat window it fills the full conversation column, the same rule the AI's own message bubble already follows — the card reads as part of that turn, not a smaller object dropped into it.
- **Rule 10 — Mobile restacks the actions by priority, not just by width.** Under 767px the card goes full-width and the three actions become one full-width button per row, ordered Allow for session (Primary) on top, Allow once (Secondary) beneath it, Deny last — the reverse of desktop's deny-first order. The underlying tab order is left exactly as desktop's (still Deny → Once → Session); only the visual stacking flips.
- **Rule 11 — Reduced motion drops the entrance and the expand/rotate transitions, keeps the same information.** Same layout, same detail, no travel.

**Do:** show the risk badge in the same corner whether pending or
resolved; keep the detail block's values complete and unabbreviated;
escalate visual commitment left to right with Deny first on desktop;
leave a resolved card visible in the transcript as a record of what
was decided; fill the host's own width inside Chat window; restack
the actions to one full-width button per row on mobile, Primary on
top. **Don't:** overlay the composer or the thread with a scrim;
collapse a request to raw JSON or truncate a value; let a resolved
decision disappear from the transcript; offer any control that
reaches the Expired state directly; stretch or center the desktop
actions row — natural width, right-aligned, same as Button renders
...
**Streaming Text.** ⚠️ **Designed from scratch** — no source in either
the original brand deck or the teammate's build.

The model's own prose, arriving word by word, with a citation landing
in position mid-sentence and the message actions/Follow-ups becoming
reachable only once the last word settles. This is what [Loading
State](#ai-native) and [Thinking](#ai-native) both hand off to at first
token — the thing either of them was standing in for. Once the stream
settles it becomes an ordinary static message; nothing about it stays
animated, and nothing about it can re-enter this state (Rule 4, Rule
22). Class prefix: `.c-streaming*`.

**Streaming Text — Anatomy.**

| Part | Spec |
|---|---|
| Root (`.c-streaming`) | Flex column, `max-width: 420px` — drop it into a chat thread's own 760px measure and it just fills to that instead. No pre-reserved height by default: appending is the default reveal, not a pre-sized box (Rule 8) |
| Text (`.c-streaming-text`) | `body1` at `line-height: 1.65` — matches [Chat window](#ai-native)'s own AI bubble exactly, its most common host. Words append as real text nodes one at a time; never a full re-render of what's revealed so far, which would tear down and replay every earlier node's own entrance animation (the citation chip's, specifically) on every new word |
| Caret (`.c-streaming-caret`) | A 2px bar reusing [Prompt input bar](#ai-native)'s own `c-prompt-bar-caret-blink` keyframe verbatim, recolored to Neutral-9 rather than the ghost text's Neutral-5. Present only while streaming; removed outright the instant the last word lands, never faded out (Rule 4) |
| Citation (`.c-streaming-cite`) | Inline chip — source avatar + domain — landing mid-sentence as a token like any other (Rule 14), not appended after the fact. `vertical-align: middle`, tied to the surrounding text's own font metrics rather than a fixed px offset, so it still centers correctly regardless of host text size |
| Note (`.c-streaming-note`) | Present only on `.is-interrupted` — "Stopped early," Neutral-5 caption, sitting ahead of the action row it unlocks (Rule 17) |
| Actions (`.c-streaming-actions`) | [Chat window](#ai-native)'s own `.c-chat-action` buttons, reused verbatim — same context, same behavior, including Copy's swap-to-a-green-check. Toggled with the `hidden` attribute, not opacity: Rule 15 requires this row to be genuinely absent from the tab order while streaming, not merely dimmed |
| Sources (`.c-streaming-sources` + trigger) | Overlapping avatar stack + a count, disclosure via Price summary card's own `0fr → 1fr` grid-rows technique, the same one Thinking's own trace already uses |
| Follow-ups (`.c-streaming-followups`) | A caption/600 label plus a list of prompts, each reusing Thinking's own row-entrance keyframe. `hidden` until settle, and never on an interrupted response — they're derived from a completed answer that, on interrupt, doesn't exist (Rule 16) |

**Streaming Text — Content types.** The reveal mechanic is chosen by
what the content actually is, not by preference:

| Content | Reveal |
|---|---|
| Prose | Word by word — the default (Rule 5) |
| Citation | Lands in position, mid-stream, as a token like any other (Rule 14) |
| List | Item by item, never word by word — a bulleted list revealed one word at a time reads as one long broken sentence (Rule 13) |
| Table / structured rows | Arrives whole, once complete — nobody reads a table in reading order (Rule 11) |
| Code block | Arrives whole; the surrounding prose still streams around it. Syntax highlighting can't run on an incomplete token anyway (Rule 12) |

**Streaming Text — Rules.** Twenty-two rules, not stylistic preference
— each one closes off a specific way a streaming reveal has been seen
to fail.

- **Rule 1 — Model-generated prose only.** This component renders text a model is actually producing. It never renders text the system computed — the Hero card's own recommendation lines come from arithmetic in `renderRevenuePeriod`, deterministic and instant, and must appear whole. Computed text is not generated text.
- **Rule 2 — Never simulate a stream over content you already hold.** If the full string is already in hand — cached, replayed, computed, or arrived in one response — render it whole. Animating a reveal over complete content makes the product measurably slower in order to look more sophisticated. That's the one thing this component must never be used for.
- **Rule 3 — Begins at the first token, not before.** Everything before the first token belongs to [Loading State](#ai-native) or [Thinking](#ai-native). This component's own arrival is what removes those.
- **Rule 4 — Ends when the last word settles, and then it is no longer this component.** On settle the text becomes an ordinary static message. Nothing about it stays animated, nothing stays in a streaming class, and nothing about it can re-enter this state.
- **Rule 5 — Reveal at word granularity, never character.** Character-by-character is a simulation of typing and reads as theatrical. Word-by-word matches both how tokens actually arrive and how people actually read.
- **Rule 6 — Cadence comes from token arrival, smoothed through a buffer.** Raw arrival is lumpy — twenty words at once, then 400ms of nothing — which reads as broken rather than fast. Hold arriving tokens and release at a steady rate, targeting ~55ms per word (about 180wpm, inside comfortable reading speed), draining faster when the buffer is backing up. The fixed 55ms is a fallback, not the mechanism.
- **Rule 7 — Below roughly 12 words, reveal the response whole.** 55ms × 12 is 660ms of animation over something that could have been instant. Short answers aren't improved by being delayed.
- **Rule 8 — Appending is the default reveal. Pre-reserved layout is the exception.** In normal top-down text flow, appending a word never moves a word already on screen — only the last line grows. In a chat thread, where nothing sits below the message, append. Reserve the final layout only where content below must not move: a card with a footer, a fixed-height container, a message followed by controls that are already on screen.
- **Rule 9 — When you do pre-reserve, use `visibility`, not opacity alone.** `opacity: 0` leaves unrevealed words selectable and in the accessibility tree — a drag-select or Ctrl+A picks up text the reader can't see. `visibility: hidden` reserves the same layout space and excludes the text from both. Pair it with opacity for the fade, giving `visibility` a `0s` delay on the way in — the same technique [Chat window](#ai-native)'s own Jump to present button already uses.
- **Rule 10 — Never justify streaming text, and never balance it.** `text-align: justify` recalculates word spacing on every line as content changes, so earlier lines shift. `text-wrap: balance`/`pretty` redistribute across the whole block for the same reason. Both break Rule 8's guarantee that nothing already read moves.
- **Rule 11 — Structured output lands whole.** Tables, comparison rows, option cards, KPI rows. Nobody reads a table in reading order, so revealing it that way fights the content. It appears as a unit once complete.
- **Rule 12 — Code blocks land whole.** A partially-revealed block is unreadable, and syntax highlighting can't run on an incomplete token — it either flickers as it re-highlights or renders bare and then jumps. The surrounding prose streams; the block arrives as a unit.
- **Rule 13 — Lists stream by item, not by word.** A bulleted list revealed word by word reads as one long broken sentence. Each item appears whole, in order.
- **Rule 14 — Citations arrive in position, as part of the stream.** Not appended at the end. A citation lands mid-sentence where it belongs, which means citation rendering can't be deferred to settle — it's stream content like any other token.
- **Rule 15 — Message actions are unavailable until settle.** Copy on half a response is a bug. Thumbs on an incomplete answer is meaningless. The action row isn't merely dimmed during the stream — it isn't rendered, so nothing in it is reachable by Tab.
- **Rule 16 — Follow-ups appear only on settle, and only after the actions.** They're derived from the completed answer, so they can't exist before it. Landing them after the action row keeps the order: the answer, then what you can do with it, then where you might go next.
- **Rule 17 — A stopped stream keeps its text.** Interruption doesn't discard what was delivered. The partial response stays, is marked as interrupted, and its action row becomes available on what exists. A stopped response is still a response, and deleting it destroys output the reader may already have read.
- **Rule 18 — A failed stream keeps its text too.** A dropped connection leaves the partial text in place with an error state and a retry beside it. Retry appends or replaces per the product's own decision, but it never silently erases what arrived.
- **Rule 19 — `aria-busy` during, announce on completion. Never a live region on the text.** `aria-busy="true"` on the message while streaming, and no `aria-live` anywhere on the streaming content — a live region over text mutating every 55ms is unusable. On settle, flip `aria-busy` to false and announce completion politely, once. A partially-arrived response isn't useful to read, and the settled one is reachable normally.
- **Rule 20 — It never scrolls anything. The thread owns that.** Streaming text doesn't call `scrollIntoView` or touch `scrollTop`. Following new content is the thread's own scroll pin, and the new-content signal is Jump to present's. Two things scrolling one container is how a reader gets yanked mid-sentence.
- **Rule 21 — Reduced motion reveals in one step.** No per-word fade, no cadence. Text appears as it arrives, in whatever chunks the buffer delivers, with no transition. The progressive arrival stays — that's the information — and only the animation of each word goes.
- **Rule 22 — No loop. No replay. No re-stream.** The stream runs exactly once, per message, ever. Scrolling an old message back into view must not restart it. A looping reveal is an animation rather than a stream, and a reader who looks away and back would watch the answer un-write itself.

**Do:** render only text a model is actually producing, whole and
instantly for anything already fully in hand; reveal prose word by
word, lists item by item, tables/code/structured rows as a single
unit; keep the action row and Follow-ups entirely unrendered — not
dimmed — until settle; keep a stopped or failed stream's partial text
in place, marked, with its own actions reachable on what exists;
flip `aria-busy` and announce once, on settle, never mid-stream.
**Don't:** animate a reveal over content already fully in hand; hold a
short, sub-12-word answer back at 55ms/word when it could render
whole; justify or balance streaming text, or let the last word's
...
| Part | Spec |
|---|---|
| Root (`.c-thinking`) | Flex column, `max-width: 380px`. `min-height` transitions between `0` and its working/expanded value (`176px`) on `duration-morph`/`ease-morph` — a pose change, not decoration, so it takes Prompt input bar's own morph pairing rather than Loading State's `ease-standard` |
...
| Element | Rule |
|---|---|
| Category axis | No border, no gridlines. On a vertical chart this is X; on a horizontal bar it is Y — **read these two rows by role, not by letter**. An earlier wording named X and Y directly, which silently assumed every chart was vertical and left horizontal bars ambiguous |
| Value axis | No border; the **only** gridline on the chart, color Neutral-3, hairline. X on a horizontal bar, Y on a vertical one |
...
| **Sparkline** | 2px | none, or the ≤12% wash off its own colour | none, except optionally the final point at 3px | **A glance, not an instrument.** No axes, no gridlines, no legend, no tooltip, no "View as table" — it is the only chart type besides Bubble exempt from that button, because it has no standalone data to tabulate: it annotates a figure that is already on screen. `tension: 0.3` as Line (plain). Takes the colour of the element that owns the metric. Lives in [Stat / KPI card](#stat--kpi-card)'s reserved sparkline slot rather than a chart card, so it inherits no chart chrome at all |
| **Stacked bar** (horizontal or vertical) | — | each segment its own colour, per the rule below | — | **Corner radius is the outer end of the stack only.** The 4px top-only rule above is written for a single bar; applied to every segment of a stack it notches the joints and the bar stops reading as one thing. Round the **last non-zero segment's outer end** — computed per bar, since which segment is last changes with the data — and leave every internal join square. Thickness comes from the same `--spacing-20`/`--spacing-24` caps as any other bar. **The stack must sum to the figure it is labelled with**: if the segments don't account for the whole, draw the remainder in Neutral-3 rather than leaving it off, which is [Progress-to-Goal Bar](#progress-to-goal-bar)'s own headroom role reused. **Colour depends on what the segments are** — genuinely different categories walk the department table per [Chart color mapping](#chart-color-mapping); parts of *one* measure (two time windows, say) are **sequential**, so they take a single-hue ramp off the owning element and never two hue families. ⚠️ A sequential ramp cannot clear 3:1 at every step — measured on the Water ramp, Navy is 4.54:1 against the card but 2.41:1 against `water-pastel`, which is itself 1.89:1 against the card. Segments are therefore identified by the legend, the tooltip, and the mandatory table view, never by colour alone |
...
| "View as table" button (`.c-chart-table-btn`) | Reuses `.c-btn c-btn-secondary c-btn-sm` verbatim — no bespoke button CSS. Every chart gets one except **Bubble** (structurally has no accompanying `<table>` at all) and **Sparkline** (it annotates a figure already on screen; there is no standalone data to tabulate) |
...
| Parts within a total, compared across categories | Stacked bar chart — and the stack must sum to the total it is labelled with |
...
| The shape of a trend beside a headline number | Sparkline in [Stat / KPI card](#stat--kpi-card)'s own slot — never a full chart card |
...
| Sparkline slot (optional) | reserved space below the trend row for a mini inline chart. Rendering **is** specified — see **Sparkline** in [Chart chrome & marks](#chart-chrome--marks)' per-chart-type table: 2px line, `tension: 0.3`, no axes, no gridlines, no legend, no tooltip and no "View as table", coloured by the element that owns the metric. [Chart color mapping](#chart-color-mapping) covers the colour role |

**Variants:** Static (default), Interactive/clickable (same hover →
`shadow-2` + focus-visible → `shadow-focus` pattern as Card),
**Negative value**, and **Secondary**.

**Negative value.** For a metric whose value is below zero — a negative
margin, a net loss, an over-budget delta.

| Part | Rule |
|---|---|
| Border | `border-width-md` (2px) solid Red `#FD3343`, with padding compensated to `calc(spacing-16 - 1px)` so the tile doesn't grow and break grid alignment — the same escalated-border recipe [Card](#card)'s own Selected state uses, in Red rather than Obsidian |
| Value | Red `#FD3343`. The only part of the card that recolors |
| Label, icon chip, trend, footer | unchanged. The trend keeps its own Red/Green, which reports **direction of change** — a different signal from the value's sign, and the two can legitimately disagree: a negative value that is improving reads as a Red border with a Green trend, and that's correct |
| Applied by | the consumer, as an explicit flag. The component can't infer sign from a formatted string like `-RM14.2k`, so whoever renders the card is responsible for keeping the flag in sync with the value |

⚠️ The Red value renders at `h1`/800 (32px). On a dashboard where
several tiles are negative at once, that is a lot of full-strength Red
at large size — check a realistic worst-case screen before using this
in a dense grid.

**Secondary stat/KPI card.** A lighter tile for a *supporting* metric —
the figure that gives a primary KPI its context (where the month is,
the run rate needed to hit target, how much of a goal is banked) rather
than the headline number itself. Same container, same padding, same
border as the default; only the type steps down.

| Part | Rule |
|---|---|
| Container | unchanged from the default card |
| Label | unchanged — caption/700, `tracking-eyebrow`, uppercase, Neutral-5 |
| Value | **h4 (20px/28px/800)**, not h1. See the override note below |
| Unit / denominator (optional) | a trailing `footnote` (12px/16px/400) Neutral-5 run inside the value, baseline-aligned with a 4px gap — either a denominator ("Day 6 **of 31**") or a rate's unit ("RM144k **a day**") |
| Goal bar (optional) | [Progress-to-Goal Bar](#progress-to-goal-bar)'s **In-table** variant — 8px `radius-pill` rail, target line at its fixed 75%, no label on the bar itself. Inside this card the In-table variant's reserved label band above the rail collapses to the target line's own overhang, since nothing sits there |
| Achieved label | when the goal bar is present, one `caption` line below the rail putting the rail's shape into words — percentage Neutral-9/700, remainder Neutral-5 (e.g. "**53%** of target achieved"). Not optional: a bare rail states a ratio it never names |
| Description (optional) | `caption`/400 Neutral-5, no divider and no top border — that's what the Footer sub-part is for |
| Trend | not used. A supporting metric is context for a comparison, not itself a comparison; if the figure genuinely has a period-over-period delta, it's a primary card |

**Sub-variants:** *with goal bar* and *without*. Without is the card
plus the type step and nothing else — no CSS of its own. With adds the
rail and its achieved label.

⚠️ **This variant deliberately overrides the "Value | h1" rule in the
anatomy table above** — the only sanctioned departure from it besides
`display` for a hero tile. Two reasons: a compound statistic like "Day
6 of 31" doesn't hold at 32px, and the step down in size *is* the
hierarchy — it's what lets a secondary tile sit in the same grid as a
primary one without competing with it. Stated here explicitly so it
doesn't read as an oversight and get "corrected" back to h1.

**Do:** pair it with a primary card in the same row — that pairing is
the whole point of the name, and the h1/h4 contrast is what carries the
hierarchy. **Do:** always caption a goal rail with its achieved
percentage. **Don't:** use Secondary just to fit a long number into a
narrow tile — if the metric is the headline, it stays h1 and the tile
gets wider. **Don't:** mix Secondary and default sizing *within* a
metric's own group where all the tiles are peers; the size difference
asserts a rank that isn't there.
...
period. **Don't:** reach for Negative value because a number is merely
falling — it marks values below zero, not bad news; a declining
positive number is exactly what the trend row is for.
...
- **v0.9.100 — 2026-09-08** — Four fixes to
  ...
  build walked into rather than review findings. **Axis rules are now
  stated by role, not by letter**: the chrome table's `X-axis` / `Y-axis`
  rows become **Category axis** / **Value axis**, because the old wording
  silently assumed a vertical chart and left a horizontal bar with no
  answer to where its one Neutral-3 gridline belongs (the value axis —
  X on a horizontal bar, Y on a vertical one). **Stacked bar** is
  specified for the first time, in both the type-selection and
  per-chart-type tables: the 4px top-only radius is written for a single
  bar and notches the joints if applied to every segment, so a stack
  rounds the **outer end of its last non-zero segment only**, computed
  per bar since which segment is last changes with the data; the stack
  **must sum to the figure it is labelled with**, drawing any remainder
  in Neutral-3 — [Progress-to-Goal Bar](#progress-to-goal-bar)'s headroom
  role reused; and segment colour depends on what the segments *are* —
  distinct categories walk the department table, while parts of one
  measure (two time windows, say) are **sequential** and take a
  single-hue ramp off the owning element rather than two hue families. A
  ⚠️ records that a sequential ramp cannot clear 3:1 at every step
  (measured on Water: Navy 4.54:1 against the card, 2.41:1 against
  `water-pastel`, which is itself 1.89:1), so segments are identified by
  legend, tooltip and the mandatory table view, never colour alone.
  **Sparkline** is specified for the first time — [Stat / KPI
  card](#stat--kpi-card) has reserved a slot for one since it was
  written while explicitly declining to define its rendering, and that
  sentence now points at a real recipe: 2px line, `tension: 0.3`, no
  axes, gridlines, legend or tooltip, coloured by the element that owns
  the metric. It becomes the **second exemption from "View as table"**
  after Bubble, on the grounds that it annotates a figure already on
  screen and has no standalone data to tabulate. `SKILL.md` gains a line
  naming Chart.js as the charting library with its CDN tag and the
  gallery's reference build, which the cheat sheet had never mentioned —
  an agent reading only `SKILL.md` had no way to know charts were
  already solved here. `components.css` and `tokens.css`: unchanged —
  all four are documentation.
...
- **v0.9.94 — 2026-09-03** — Added a **Secondary** variant to
  ...
  the headline number itself. Container, padding and border are
  untouched; the statistic steps down from `h1` (32px) to `h4` (20px),
  and that step *is* the hierarchy — it's what lets a secondary tile
  share a grid with a primary one without competing with it. Two
  sub-variants: with [Progress-to-Goal Bar](#progress-to-goal-bar)'s
  In-table rail (plus a mandatory caption below it naming the achieved
  percentage — a bare rail states a ratio it never names), and without,
  which is the type step and nothing else and needs no CSS of its own.
  Also introduced: an optional trailing `footnote` run inside the value
  for a denominator or a unit ("Day 6 **of 31**", "RM144k **a day**"),
  and `.c-stat-desc`, a plain caption description line — left unscoped
  since any Stat card can use it, and distinct from the existing Footer
  sub-part, which adds a divider and a top border. Inside this card the
  In-table rail's reserved label band collapses to the target line's own
  overhang, since nothing sits above the rail here. **This variant
  knowingly overrides the component's own "Value | h1" anatomy rule** —
  the only departure from it besides `display` for a hero tile — and the
  section says so out loud with the reasoning, so it doesn't read as an
  oversight and get reverted to h1 by someone rebuilding it. The trend
  row is deliberately excluded rather than merely optional: a supporting
  metric is context for a comparison, not itself one, and a figure with
  a real period-over-period delta belongs in a primary card. No new
  global tokens — `tokens.css` is unchanged.
- **v0.9.93 — 2026-09-03** — Added a **Negative value** variant to
  [Stat / KPI card](#stat--kpi-card), for a metric whose value is below
  zero: the card border escalates to 2px Red `#FD3343` and the value
  itself turns Red. Nothing bespoke — the border weight and its 1px
  padding compensation are [Card](#card)'s own Selected-state recipe
  (`--border-width-md` + `calc(spacing-16 - 1px)`, so the tile can't
  grow and knock a KPI grid out of alignment), reused in Red instead of
  Obsidian, and the Red is the same functional value Badge Danger,
  Progress Bar's Error state and Input field's error border already
  carry. Considered and rejected: a 1px border (Search input's
  `.is-error` weight) — at tile scale, 1px Red against 1px Neutral-3
  reads as a rendering artifact rather than a signal — and a tinted
  card background, which gets loud fast in a grid.
  Deliberately scoped to the border and the value only: the label, icon
  chip, trend and footer keep their existing treatment. In particular
  the trend stays on its own Red/Green, because it reports direction of
  change rather than the value's sign, and the two can legitimately
  disagree — a negative value that's improving is a Red border with a
  Green trend, which is correct rather than a conflict. The variant is
  a consumer-applied flag, not derived: the component can't read a sign
  off a formatted string like `-RM14.2k`. Flagged in the section, not
  hidden: the Red value sits at `h1`/800, so a dashboard with several
  negative tiles at once carries a lot of full-strength Red at 32px and
  is worth checking against a real worst-case screen. A "don't" line
  was added alongside it — this marks values below zero, not bad news;
  a declining positive number is what the trend row is already for.
...
- **v0.9.90 — 2026-09-02** — `preview.html` only, no component
  ...
  examples (Low/Medium/High risk, plus one pre-Expired) down to one —
  the Low-risk "Read pipeline-config.ts?" example only — and drops the
  explanatory caption paragraph that sat under its swatch-group-label,
  the same trim [Streaming Text](#ai-native) and
  [Thinking](#ai-native)'s own galleries already went through
  (v0.9.77). `mountPermissionPrompt` itself is unchanged and still
  accepts any risk tier or initial state; only the gallery's own mount
  calls were reduced to the one example. `components.css`/
  `tokens.css`: unchanged — this is a demo-scope change only, the
  component's own eleven rules and anatomy are untouched.

- **v0.9.89 — 2026-09-02** — Added **Permission Prompt**, [AI
  Native](#ai-native)'s seventh subcomponent, alphabetically between
  Loading State and Prompt input bar in the flat list this component
  is otherwise pulled from: a human-in-the-loop card the AI surfaces
  inline, in its own turn, to ask authorization for a specific,
  consequential action — reading or writing a file, running a
  command, sending something on the person's behalf, calling an
  external API — before it proceeds. Formalized from a draft
  (`preview-permissionprompt-draft.html`, now deleted) built up over
  several rounds of review: full width inside Chat window's
  conversation column, right-aligned deny-first actions on desktop,
  center-aligned icon/label/badge and a heading-5 label matching
  [Tool approval](#ai-native)'s own size, and — the change that
  triggered this formalization pass — mobile actions restacking to
  one full-width button per row, Primary (Allow for session) on top,
  Secondary (Allow once) beneath it, Deny last, reversing desktop's
  deny-first order while leaving the underlying tab order untouched.
  Eleven rules supplied directly for this commit — scope (a specific
  consequential action, never a general confirmation), inline
  placement rather than a modal, the detail defaulting open while
  pending and staying reachable once resolved, structured key:value
  detail rows that never truncate and are never raw JSON, a risk
  badge that stays in the same corner pending or resolved, deny-first
  escalating commitment across the three actions, a resolution that
  stays visible as a permanent audit trail (a deliberate contrast
  with Tool approval's own Rule 13 — that flow closes outright on
  resolution, since it's answering questions rather than authorizing
  a consequential action), Expired as a session boundary no control
  reaches directly, width matching the host, the mobile restack
  described above, and a reduced-motion pass. `components.css` gets
  the new `.c-permission-prompt*` CSS — reusing Card, Badge's real
  status variants, Button, and Thinking's own `0fr → 1fr` grid-rows +
  opacity expand/collapse technique for the detail block, verbatim,
  no new controls invented. Two real gaps in the system, carried over
  from the draft rather than resolved here: no `.c-btn-danger` exists,
  so Deny reuses Table's own established recipe (Ghost + a scoped
  `--color-red` override) instead of a new Button variant; no
  dedicated muted/expired Badge tone exists beyond `.c-badge-neutral`,
  so the Expired state uses that. `preview.html` gains a real,
  interactive gallery entry (`#permissionPromptDemo`) — four
  live examples (Low/Medium/High risk, plus one already Expired) — its
  own Chat window instance proving the component as a genuine child of
  `.c-chat-msg.is-ai`, and a real 375px mobile iframe for both.
  `tokens.css` unchanged — every value this component needed
  (`--text-h5-*`, `--icon-md`, `--icon-micro`, `--color-red`, the
  existing Badge status tokens) already existed. Net component count
  unchanged — subcomponents of AI Native aren't separately counted,
  same as every other addition to this category before it.
...
- **v0.9.76 — 2026-08-28** — Added **Streaming Text**, [AI Native](#ai-native)'s
  ...
  prose, arriving word by word, with a citation landing in position
  mid-sentence and the message actions/Follow-ups becoming reachable
  only on settle. Twenty-two new rules supplied directly for this
  commit — scope (model-generated prose only, never a simulated reveal
  over content already in hand), begins at first token, ends at settle
  and can't re-enter the streaming state, word-not-character
  granularity, a buffer-smoothed ~55ms/word fallback cadence,
  instant whole-reveal below ~12 words, append-by-default layout with
  pre-reserved space as the documented exception, no justify/balance,
  structured content (tables/lists/code) landing whole or by item
  rather than by word, in-position citations, a genuinely unrendered
  (not dimmed) action row until settle, Follow-ups only after the
  actions, a stopped or failed stream keeping its partial text marked
  rather than discarded, `aria-busy` with a single settle announcement
  and no live region on the mutating text itself, never touching thread
  scroll, a one-step reduced-motion reveal, and no loop/replay/re-stream
  ever. All twenty-two documented under the new **Streaming Text —
  Rules** heading, plus a **Streaming Text — Content types** table
  covering Rules 11–14. Loading State's own Rule 6 updated to link here
  instead of the plain "streaming is the text itself" it shipped with.
  `components.css` gets the new `.c-streaming*` CSS — reusing [Chat
  window](#ai-native)'s own `.c-chat-action` buttons verbatim for the
  action row, [Prompt input bar](#ai-native)'s own caret-blink keyframe
  for the cursor, Thinking's own fade-in/row-entrance keyframes for the
  citation chip and Follow-up rows, and Price summary card's `0fr → 1fr`
  technique for the sources disclosure — no new motion primitives.
  `preview.html`'s gallery entry sits between Prompt input bar and
  Thinking (alphabetical, per this component's own slot). It loops
  automatically for the gallery's own visibility; Rule 22 means a real
  instance never does — flagged directly in the gallery caption rather
  than silently diverging. Its Stop button is real, not decorative: it
  interrupts the sequence mid-stream, the partial text stays and gets
  marked "Stopped early," and the action row unlocks on what's there,
  demonstrating Rule 17 live rather than only in prose. Net component
  count unchanged — subcomponents of AI Native aren't separately
  counted, same as Chat window/Loading State/Prompt input bar/Thinking
  before it. `tokens.css` unchanged: every value this component needed
  already existed.
